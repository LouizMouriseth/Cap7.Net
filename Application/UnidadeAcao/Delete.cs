...
public class Delete
{
    ...
    internal class Service : IRequestHandler<Request, BaseResponse<ViewModel>>
    {
        ...
        {
            var unidadeAcao = await _uow.UnidadeAcaoRepository.GetByIdAsync(request.Id, ct);
            
            if (unidadeAcao == null)
                return new NoDataResponse<ViewModel>(404, "UnidadeAção não existe");

            await _uow.UnidadeAcaoRepository.DeleteAsync(request.Id, ct);
            await _uow.CommitAsync(ct);
            
        return new NoDataResponse<ViewModel>("UnidadeAção excluída");
        }
    }
}